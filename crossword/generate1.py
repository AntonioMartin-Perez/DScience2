...
class CrosswordCreator():
    ...
    def backtrack(self, assignment):
        ...
        neighborhoodArcs = list()
        for neighbor in neighborhood:
            neighborhoodArcs.append(
                (var,neighbor)
            )

        if self.ac3(neighborhoodArcs):
            varOrderedDomain = self.order_domain_values( var,assignment )
            for value in varOrderedDomain:
                # If value consistent with assignment
                # if len(value) == var.length:

                # Creo que tengo que verificar el arc consistency aquí de la nueva variable con sus neighbors
                # arcs = list()
                # for variable in assignment.keys():
                #     arcs.append(
                #         (variable,var)
